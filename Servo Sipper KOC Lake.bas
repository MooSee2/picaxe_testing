...
symbol Lift_BIN1 = C.1			' drive filtration pump with C.0 and C.1
symbol Lift_BIN2 = C.0
...
'symbol Servo_number_0 = B.0		' single 8-valve servo, on B port
'symbol Servo_number_1 = B.1		' single 8-valve servo, on B port
'symbol Servo_number_2 = B.2		' single 8-valve servo, on B port
'symbol Servo_number_3 = B.3		' single 8-valve servo, on B port
...
symbol Outlet_Open_position = 150               'stopcock arm up
symbol Outlet_Close_position = 50                 ' stopcock arm to left
symbol Valve_Wakeup_interval = 5                                                               
symbol Valve_Wakeup_time = w9
...
'	gosub Get_Li_Batt	
	...
	sertxd (CR,LF,CR,LF,"ServoSipper-Koocanusa-Slot#0-Stopcock Outlet+Wakeup-Sep1-2022",CR,LF)
...
'      if RTC_flag = 0 then
      	...
      	gosub Display_Time
'         else     			' download data if reset with RTC failure

	
'		sertxd (CR,LF,CR,LF,"RTC FAIL",CR,LF)
'		sertxd (CR,LF,"6)DOWNLOAD EEPROM",CR,LF)

'		sertxd (CR,LF,"8)RESET RTC_FLAG",CR,LF)
...
'	IF J = 7 then	GOSUB Set_Clock     		' Goto Display time subroutine



'	IF J = 8 THEN     gosub Reset_EEPROM
...
Check_Parameters:
'     if Coil_Fill_time <2 then           ' check to see if Sample_pulses loaded correctly
...
Pump_Servo_Test:
	...
	SerTxd (CR,LF,CR,LF,"TESTS 99=exit")     ' Get input	gosub Exit_99
	SerTxd (CR,LF,"1)Move Servo")    ' Get input
'	SerTxd (CR,LF,"2)Sample Pump pulses")        ' Get input
      SerTxd (CR,LF,"3)Sample Pump")     ' Get input
'	SerTxd (CR,LF,"4)Drain")     ' Get input

	SerTxd (CR,LF,"5)Open/Close Outlet")     ' Get input
'	SerTxd (CR,LF,"6)Cycle valve")     ' Get input
	SerTxd (CR,LF,"7)Wet Filters")     ' Get input
	SerTxd (CR,LF,"8)Sample Fill")     ' Get input
'	SerTxd (CR,LF,"9)check valve pos")     ' Get input
'	SerTxd (CR,LF,"10)Run Fan",CR,LF)     ' Get input
'	SerTxd (CR,LF,"10)Sleep current",CR,LF)     ' Get input
...
'		J=0
	...
	if J = 5 then
		SerTxd (CR,LF,"0=Open, 1=Close")     ' Get input
		SerRxd [65535,Pump_Servo_Test], #J
		If J = 0 then 
			gosub Open_Outlet_valve
		  else
	...
	If J=6 then
'		SerTxd (CR,LF,"Select Servo# (0-15)")     ' Get input
'		SerRxd [65535,Pump_Servo_Test], #Servo_number
'		if Servo_number <16 then
'			for J = 1 to 3
'				gosub Servo_ON    
'				Servo_position = Valve_8
'				gosub Move_Servo
'				Servo_position = Valve_0
'				gosub Move_Servo
'			next J
'		 else						' cycle all valves
'			gosub Servo_ON    		' turn on I2C
'			for Servo_number = 0 to 15
'				Servo_position = Valve_8
'				gosub Move_Servo
'				Servo_position = Valve_0
'				gosub Move_Servo
...
'		gosub I2C_OFF
	...
	if J = 7 then
'		SerTxd (CR,LF,CR,LF,"Enter #Valves")     ' Get input
'		SerRxd [65535,Pump_Servo_Test], #Z
'		SerTxd (CR,LF,"#Vales= ",#Z)     ' Get input

		gosub Enter_Pump_pulses
		gosub Close_Outlet_valve
'		for Servo_number = 0 to Z
...
'	if J = 9 then goto Main_Menu  		' return to Main Program
...
'	sertxd (CR,LF,CR,LF,"Enter Outlet Open position (75-100)")
'	serrxd [65535,Main_Menu], #Outlet_open_position
	gosub Servo_ON
...
Close_All_Valves:
  ...
  For Servo_number = 0 To 15
	...
	Gosub Move_Servo
  Next
  gosub I2C_OFF		'HAVE TO POWER OFF I2C BEFORE SELECTING Sample FILL VALVE, NOT SURE WHY
return
...
Deep_Sleep:
	...
	Valve_Wakeup_time = Countdown - Valve_Wakeup_interval		' set initial wakeup time
...
Wakeup_Valves:
	sertxd (CR,LF,"Wakeup Valves")
	gosub Close_Outlet_Valve
	gosub Open_Outlet_Valve
	Valve_Wakeup_time = Countdown - Valve_Wakeup_interval		' reset wakeup time
...
Minute_Countdown:
	...
	do while Countdown > 1  		' exits countdown 1 minute before wakeup 
		sertxd (CR,LF,"Countdown= ",#Countdown,", Wake= ",#Valve_Wakeup_time)
		...
		If Countdown = Valve_Wakeup_time then gosub Wakeup_Valves	
...
'	hi2csetup i2cmaster,RTC_address,i2cslow,i2cbyte		' set i2c for DS1340

'	sertxd (CR,LF,CR,LF,"Enter date/time: YY, MM, DD, HH, mm, ss",CR,LF)

'	for J = 6 to 0 step-1
'		sertxd ("Enter Value",CR,LF)
'		if J = 3 then 
'			J=2			' skip day of week
'		endif
'		serrxd [65535,Main_Menu], #b0
'		sertxd ("J = ",#J,", Value = ",#b0,CR,LF)
'		b0 = BINTOBCD b0
'		hi2cout J, (b0)
...
'     read 10, word Pump_pulse_ON_time
'   read 12, word Pump_pulse_OFF_time
